...
function EditProduct() {
    ...
    const onSubmit = data => {
        clienteAxios.put('product/'+id, data)
            .then(resp =>{
                succesSwal()
                navigate(`/productos`);
            })
                ...
                <SoftBox p={2} component="form" role="form" onSubmit={handleSubmit(onSubmit)}>
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={6} xl={6}>
                            <SoftBox mb={2}>
                                <TextField fullWidth label="Nombre" variant="outlined" defaultValue={store.name} 
                        ...
                        <Grid item xs={12} md={6} xl={6}>
                            <SoftBox mb={2}>
                                <TextField fullWidth label="Direccion" variant="outlined" defaultValue={store.address} 
                        ...
                        <Grid item xs={12} md={6} xl={6}>
                            <SoftBox mb={2}>
                                <TextField fullWidth label="Telefono" variant="outlined" defaultValue={store.phone} 
                        ...
                        <Grid item xs={12} md={6} xl={6}>
                            <SoftBox mb={2}>
                                <select className="form-select" defaultValue={store.status} {...register("status", { required: true })}>
                    ...
                    <SoftBox mt={4} mb={1}>
                        <SoftButton type="submit" variant="gradient" color="dark" style={{float:"right"}} >Editar</SoftButton>
                    </SoftBox>
                </SoftBox>
            </Card>
          </SoftBox>
